...
class TimeWindowSampleFrames_all:
    ...
    def _get_train_clips(self, num_frames, clip_len,num_keypoint,num_person,real_num_person,kp):
        ...
        sv_boneC_next=self._boneC_next_sv(num_frames,num_keypoint,num_person,real_num_person,kp)
        sv_boneL_next=self._boneL_next_sv(num_frames,num_keypoint,num_person,real_num_person,kp)
        sv_boneC_avr=self._boneC_avr_sv(num_frames,num_keypoint,num_person,real_num_person,kp)
        sv_boneL_avr=self._boneL_avr_sv(num_frames,num_keypoint,num_person,real_num_person,kp)
        ...
        theta=[6.928,6.928,6.869,6.869,6.869,6.869,7.026,7.026,7.026,7.026] 
        ...
        sv_B1=list(np.add([(theta[2]/sum(theta)/max(float("1e-8"),max(sv_boneC_next)))*x for x in sv_boneC_next],[(theta[3]/sum(theta)/max(float("1e-8"),max(sv_boneL_next)))*x for x in sv_boneL_next]))
        sv_B2=list(np.add([(theta[4]/sum(theta)/max(float("1e-8"),max(sv_boneC_avr)))*x for x in sv_boneC_avr],[(theta[5]/sum(theta)/max(float("1e-8"),max(sv_boneL_avr)))*x for x in sv_boneL_avr]))
        ...
        sv_B=list(np.add([x for x in sv_B1],[x for x in sv_B2]))
        sv_A=list(np.add([x for x in sv_A1],[x for x in sv_A2]))
        sv=list(np.add(np.add([x for x in sv_J],[x for x in sv_B]),[x for x in sv_A]))
