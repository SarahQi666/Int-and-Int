...
def init_recognizer(config, checkpoint=None, device='cuda:7', **kwargs):
    ...
    if 'use_frames' in kwargs:
        warnings.warn('The argument `use_frames` is deprecated PR '
...
def inference_recognizer(model, video, outputs=None, as_tensor=True, **kwargs):
    ...
    Args:
        ...
        as_tensor (bool): Same as that in ``OutputHook``. Default: True.（Determine to return a tensor or a numpy array.）
